...
7) **레이아웃(타이틀·버튼 위치·정렬·여백)은 유사 패턴에서 잡는다(임의 배치 금지)**: 요청과 가장 유사한 패턴/화면의 배치를 그대로 따른다. 기본 규칙 — 타이틀=좌상단 title 스타일 첫 블록(또는 유사 패턴의 타이틀 위치), 확인/취소·주요 액션 버튼=**화면 하단**(전체를 세로 \`group{"align":"spaceBetween"}\`으로 감싸 [상단 콘텐츠 group, 하단 버튼 group] 2자식), 좌측 메뉴=settingScreen. 정렬·여백·간격도 유사 패턴 값을 재현하고 새로 지어내지 말 것.
