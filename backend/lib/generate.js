...
6) **자연어 조립 시 알맞은 컴포넌트를 쓴다(text/Image로 흉내내지 말 것)**: 툴팁·말풍선=**Tooltip**(direction=꼬리방향), 스크롤바=**Scrollbar**, 라벨 라디오 행=**RadioRow**, 아이콘/텍스트+토글 설정행=**ToggleButton**, LNB 메뉴 항목=**LnbMenuItem**, 상단 경로/뒤로=**SettingNav**, 기기 목록 행=**SettingDeviceRow**, 포스터 썸네일 카드=**ThumbnailItem**, 구독 상품 버튼=**SubProductBtn**, 마이메뉴 버튼=**MyMenuBtn**. 특히 "툴팁으로/말풍선/~하고 안내"는 반드시 Tooltip 컴포넌트.
7) **하단 정렬**: 특정 요소(버튼줄 등)를 화면 하단에 붙이려면 화면 전체를 세로 \`group{"direction":"vertical","align":"spaceBetween"}\`으로 감싸 자식 2개 [상단 콘텐츠 group, 하단 버튼 group]로 넣는다(첫=상단, 끝=하단). spacer로 억지로 밀지 말 것.
